...
void encrypt(int establishedConnectionFD);
char* getCipherText(char* mergedText, int mergedTextLength);
char* createCipherText(char* text, char* key);
void sendText(char* cipherText, int establishedConnectionFD);
    ...
    while (1) {
        ...
        if (establishedConnectionFD > 0) {
            ...
            if (pid < 0) { 
                ...
                // start the main encryption process after fork
...
void encrypt(int establishedConnectionFD) {
    ...
    // receive text and key and put into merged text
    ...
    char* cipherText = getCipherText(mergedText, mergedTextLength);

    // send it to client
    sendText(cipherText, establishedConnectionFD);
    return;
}
...
void sendText(char* cipherText, int establishedConnectionFD) {
    ...
    // receive parts/all of the value sent from the client and place it into c
    while (i < strlen(cipherText)) {
        buffer[i%bufferLength] = cipherText[i];

        // if end, then send 
        if (cipherText[i+1] == '\0' || (i+1)%bufferLength == 0) {
...
char* getCipherText(char* mergedText, int mergedTextLength) {
    ...
    return createCipherText(text, key);
}

char* createCipherText(char* text, char* key) {
    int i;
    int keyNum, textNum, cipherNum;
    char cipherChar;
    ...
    for (i = 0; i < strlen(text);i++) {
        ...
        // convert into cipher integer
        cipherNum = (textNum + keyNum)%27;

        // convert this cipher integer into a char to place into array
        if(cipherNum != 26) cipherChar = intToChar(cipherNum);
        else cipherChar = ' ';
